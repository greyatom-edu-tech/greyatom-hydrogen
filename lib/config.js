'use babel';
import CommitLive from './commit-live';

const Config = {
  getJson(key, _default = {}) {
    if(key === 'gateways'){
      return JSON.parse(JSON.parse(localStorage.getItem(CommitLive.localStoragekey)).servers.jupyter.gateway_config)
    }
    const value = atom.config.get(`greyatom-hydrogen.${key}`);
